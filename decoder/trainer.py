...
def crossval_epochs(cv, epochs_data, labels, cls, label_names=None, do_balance=None, n_jobs=None, ignore_thres=None, decision_thres=None):
    ...
    return np.array(scores), cm_txt, np.mean(scores), np.mean(f1s)
...
def cross_validate(cfg, featdata, cv_file=None):
    ...
    # Do it!
    timer_cv = qc.Timer()
    scores, cm_txt, acc, f1 = crossval_epochs(cv, X_data, Y_data, cls, cfg.tdef.by_value, cfg.CV['BALANCE_SAMPLES'], n_jobs=cfg.N_JOBS,
    ...
    if 'export_result' in cfg.CV_PERFORM[selected_cv] and cfg.CV_PERFORM[selected_cv]['export_result'] is True:
        if cv_file is None:
            if cfg.EXPORT_CLS is True:
                ...
                fout = open('%s/classifier/cv_result_%s.txt' % (cfg.DATA_PATH, cfg.REPORT_NAME), 'w') #modified by Icare SAKR
            else:
                fout = open('%s/classifier/cv_result_%s.txt' % (cfg.DATA_PATH, cfg.REPORT_NAME), 'w') #modified by Icare SAKR
        else:
            fout = open(cv_file, 'w')
...
def batch_run(cfg_file):
    ...
    acc, f1 = run(cfg, interactive=True)
    return acc, f1
...
def run(cfg, queue=None, interactive=False, cv_file=None, feat_file=None):
    ...
    if cfg.CV_PERFORM[cfg.CV_PERFORM['selected']] is not None:
        acc, f1 = cross_validate(cfg, featdata, cv_file=cv_file)
        return acc, f1
